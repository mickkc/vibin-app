...
class ThemeGenerator {
  ...
  static ThemeData generateTheme(Color primaryColor, CustomThemeSpec spec) {
    final base = ThemeData(
      ...
      colorScheme: ColorScheme(
        ...
        onPrimary: getForegroundColor(primaryColor, spec.backgroundColor, spec.foregroundColor),
        primaryContainer: blendColors(primaryColor, spec.backgroundColor, 0.2),
        onPrimaryContainer: getForegroundColor(primaryColor, spec.backgroundColor, spec.foregroundColor),
        ...
        onPrimaryFixed: getForegroundColor(primaryColor, spec.backgroundColor, spec.foregroundColor),
        onPrimaryFixedVariant: spec.foregroundColor.withAlpha(179),
        secondary: spec.secondaryColor,
        onSecondary: getForegroundColor(spec.secondaryColor, spec.backgroundColor, spec.foregroundColor),
        secondaryContainer: blendColors(spec.secondaryColor, spec.backgroundColor, 0.2),
        onSecondaryContainer: getForegroundColor(spec.secondaryColor, spec.backgroundColor, spec.foregroundColor),
        ...
        onSecondaryFixed: getForegroundColor(spec.secondaryColor, spec.backgroundColor, spec.foregroundColor),
  ...
  static getForegroundColor(Color backgroundColor, Color c1, Color c2) {
    final bgLuminance = backgroundColor.computeLuminance();
    final c1Luminance = c1.computeLuminance();
    final c2Luminance = c2.computeLuminance();

    if (bgLuminance < 0.5) {
      return c1Luminance > c2Luminance ? c1 : c2;
    }
    else {
      return c1Luminance < c2Luminance ? c1 : c2;
    }
